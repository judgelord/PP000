...
This requires installing some packages:
...
The automated check returned some issues with quarto defaults, which I fixed with JavaScript [patches](#patches) in `assets/accessibility-fixes.js`.
...
To make callout chunks accessible, I added a JavaScript patch to label it as a button: 
...
This seems to be relate to how the navigation bar is constructed (by quarto defaults, I did not modify this): 

The HTML originally included: `aria-controls="navbarCollapse" role="menu" aria-expanded="false" aria-label="Toggle navigation"`
...
This also seems to be relate to how the navigation bar is constructed (by quarto defaults, I did not modify this): 
...
This issue seems to have been fixed by adding caused by `<script>` to `assets/accessibility-fixes.js`.
