...
You can also use the [site improve Chrome extension](chrome-extension://djcglbmbegflehmbfleechkjhmedcopn/signup.html). 
...
- More on how to make MathJax content universally interpretable, aligning with 
WAI-ARIA standards: <https://app.studyraid.com/en/read/15521/539779/accessibility-considerations>

You don't need `\tag{}` or `\label{}` if you have proper equation labels like `{#eq-1a}` above.
...
Normally, when rendering qmd documents to PDF via LaTeX, you can add lines to the LaTeX preamble using `include-in-header:` in the YAML header. However, this probably does not work for lines that need to go before `\documentclass`. For example, this causes an error: 
...
Thus, I believe that a [LaTeX template](https://quarto.org/docs/journals/templates.html) is required to allow tagging when rendering qmd documents as PDFs via LaTeX (please tell me if this is wrong). 
