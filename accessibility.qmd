...
- You can also use the [Siteimprove Chrome extension](chrome-extension://djcglbmbegflehmbfleechkjhmedcopn/signup.html). 

## Math is much more accessible in HTML than PDF

Otherwise, writing equations in [MathJax](https://docs.mathjax.org/en/latest/basic/accessibility.html) is [similar to $\LaTeX$](https://quarto.org/docs/authoring/markdown-basics.html#equations).
...
Display mode allows equation labels (see @eq-1a):
$$\sum_{i=0}^n i^2 = \frac{(n^2+n)(2n+1)}{6}$$ {#eq-1a}


<!--
Alternatively, the theorem environment also allows cross-referencing: 
...
- [Math Best Practices](https://accessibility.umich.edu/how-to/math/math-best-practices)  
...
[MathJax](https://www.mathjax.org) is set of open-source JavaScript libraries that enable MathML content in browsers and additional accessibility features from the American Mathematical Society (AMS) and the Society for Industrial and Applied Mathematics (SIAM).

- [Accessibility Features](https://docs.mathjax.org/en/latest/basic/accessibility.html)
- [How to make content universally interpretable, aligning with  WAI-ARIA standards](https://app.studyraid.com/en/read/15521/539779/accessibility-considerations)



# PDF and Docx {.smaller}
...
Accessible PDFs require tagging. For LaTeX, this means adding `\DocumentMetadata{tagging=on}` before `\documentclass` and requires installing some packages.^[see [LaTeX Tagging Project](https://latex3.github.io/tagging-project/documentation/usage-instructions) ]
...
Thus, I believe a [LaTeX template](https://quarto.org/docs/journals/templates.html) is required to allow tagging when rendering qmd documents as PDFs via LaTeX.
...
- [Quarto Accessiblity](https://remlapmot.github.io/post/2025/quarto-accessibility/)
...
- [Quarto developer discussion on Web Content Accessibility Guidelines (WCAG) in revealjs](https://github.com/orgs/quarto-dev/discussions/8621) (these issues seem to have been fixed, but it is a good example of how the open source community is addressing WCAG issues flagged by tools like Siteimprove)
